...
#define NUM_GREEDY 2     // Number of greedy astronomers
...
int ordering[NUM_ASTRONOMERS];  // 0 means symmetric, 1 means asymmetric, 2 = Greedy
...
void pickup(int Phil_ID_NUM, int PHIL_TYP){
    ...
    if(PHIL_TYP == SYMMETRIC_PHILOSOPHER){
    ...
    }else if (PHIL_TYP == GREEDY_PHILOSOPHER){
        // variable to hold amount of elapsed time in seconds
        double elapsedTime = 0;
        ...
        if(!obtained) {
            // wait
            printf("Greedy Philosopher #%d could not pick up any chopsticks. He is now thinking while he waits...\n\n", Phil_ID_NUM);
            wait(Phil_ID_NUM);
        }
    }
...
void putdown(int Phil_ID_NUM, int PHIL_TYP){
    ...
    if(PHIL_TYP == SYMMETRIC_PHILOSOPHER){
        ...
        printf("Greedy philosopher # %d has put down the left chopstick!\n", Phil_ID_NUM);
    }
    ...
    if(PHIL_TYP == SYMMETRIC_PHILOSOPHER){
        ...
        printf("Greedy philosopher # %d has put down the right chopstick!\n", Phil_ID_NUM);
    }
...
void think(int Phil_ID_NUM, int TYPE) {
    ...
    }else if (TYPE == GREEDY_PHILOSOPHER){
        printf("Greedy Philosopher #%d is Thinking for %d second\n\n", Phil_ID_NUM, THINKING_TIME);
    }
...
void eat(int philosopher, int TYPE) {
    ...
    if(TYPE == SYMMETRIC_PHILOSOPHER){
        ...
        printf("Greedy philosopher # %d has picked up the left chopstick!\n", philosopher);
    }
    ...
    if(TYPE == SYMMETRIC_PHILOSOPHER){
        ...
        printf("Greedy philosopher # %d has picked up the right chopstick!\n", philosopher);
    }
    ...
    if(TYPE == SYMMETRIC_PHILOSOPHER) {
        ...
        printf("Greedy Philosopher #%d is eating for %.2f second\n\n", philosopher, eatTime);
    }

    // let the philosopher eat for eatTime seconds
    sleep((int)eatTime);

    // if greedy philosopher eat a second time
    if(TYPE == GREEDY_PHILOSOPHER){
        eatTime = ((rand()/(float)RAND_MAX)*2*AVG_EAT_TIME);
        printf("Greedy Philosopher %d is eating a second time for %.2f seconds!\n\n", philosopher, eatTime);
        sleep((int) eatTime);
    }
    ...
    if(TYPE == SYMMETRIC_PHILOSOPHER) {
        ...
        printf("Greedy Philosopher #%d is finished eating\n\n", philosopher);
    }
}
...
void place_astronomers(int *philOrdering){
    ...
    // 0 means symmetric, 1 means asymmetric, 2 = Greedy

    // first check if user entered the correct number of philosophers
    if((NUM_ASYMMETRIC + NUM_GREEDY) > NUM_ASTRONOMERS){
        perror("Error: The number of Asymmetric Philosophers + Greedy Philosophers must be less than the total number of Philosophers.\n"
               " Program exiting...\n\n");
        exit(1);
    }

    // first put in the Asymmetric
    for (int i = 0; i < NUM_ASYMMETRIC; i++) {
        ordering[i] = ASYMMETRIC_PHILOSOPHER;
    }

    // putting in Greedy Philosophers
    for(int i = NUM_ASYMMETRIC; i <= (NUM_ASTRONOMERS - NUM_GREEDY); i++){
        ordering[i] = GREEDY_PHILOSOPHER;
    }

    // putting in Symmetric Philosophers
    for (int i = (NUM_ASYMMETRIC + NUM_GREEDY); i < NUM_ASTRONOMERS; i++) {
        ordering[i] = SYMMETRIC_PHILOSOPHER;
    }
...
int main(){
    ...
    printf("Starting A Cosmic Dining Experience milky-way chopsticks Program (bonus.c bonus version). To stop the program press ctrl + c\n\n");
